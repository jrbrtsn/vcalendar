...
const char*
STR_percent_decode(STR *self, const char *src, size_t src_len)
/**************************************************************
 * decode the src string with percent (%) style escapes into self.
 ...
 */
{
   ...
   /* Work through each character in source */
   for(size_t i= 0; i < src_len; ++i) {

      /* Check for possible % encoding */
      if(i + 2 < src_len &&
         '%' == src[i] &&
         isxdigit(src[i+1]) &&
         isxdigit(src[i+2]))
...
//         assert(c[1] && c[2] && c[3]);
         if(!(c[1] && c[2] && c[3])) {
            eprintf("ERROR: encountered invalid UTF8 sequence.");
            return NULL;
         }
...
const char*
STR_escapeRegex(STR *self, const char *src)
/**************************************************************
 * Escape any characters in src such that the result can be
 * used in a regex
 ...
 */
{
   ...
   for(pc= src; *pc; ++pc) {
      switch(*pc) {
         ...
         case '$':
         case '-':
// FIXME: probably missed some special characters
            STR_putc(self, '\\');
            break;
...
const char*
STR_escapeSqlStr(STR *self, const char *src)
/**************************************************************
 * Escape any characters in src such that the result can be
 * used in an SQL data string.
 *
 * Return self's buffer.
 * NOTE: self does not get reset!
 */
...
const char*
STR_catAnyChar(STR *self, const char *src)
/**************************************************************
 * Make a string with isprint() characters in tact, and all
 * others converted to <0xAB> hex format.
 *
 * Return self's buffer.
 * NOTE: self does not get reset!
 */
{
   ...
   for(pc= src; *pc; ++pc) {
      if(isspace(*pc)) 
         ...
         STR_append(self, anychar2str(*pc), -1);
   }
   return STR_str(self);
}

int
STR_appendFile(STR *self, const char *fname)
/**********************************************************************************
 * Append contents of file 'fname' to the current buffer.
 * Returns -1 for error.
 */
{
   int rtn= -1;
...
#endif
   if(!fh) {
      sys_eprintf("ERROR: File \"%s\"", fname);
      goto abort;
   }

   // Append contents of buffer
   while(0 < (nBytes= fread(buf, 1, sizeof(buf), fh))) {
      if(-1 == STR_append(self, buf, nBytes))
         goto abort;
   }

   // Make sure end-of-file was reached
   if(!feof(fh)) {
      sys_eprintf("ERROR: File \"%s\"", fname);
      goto abort;
   }
